function varargout=sigstar_PTE(groups,stats,nosort)
    ...
    y=ylim;
    yd=0.07;%0.17;%0.07; %separate sig bars vertically by 5% 
    %yd=myRange(y)*0.05;
    %Anup changed above
    for ii=1:length(groups)
    ...
    %for all bars.
    yd=0.001;%0.03; %myRange(ylim)*0.01;%Ticks are 1% of the y axis range
    %Anup changed above
    for ii=1:length(groups)
        y=get(H(ii,1),'YData');
        y(1)=y(1)-yd;
...
function H=makeSignificanceBar(x,y,p)
    ...
    starY=mean(y)+0.01;%mean(y)+0.04;%starY=mean(y)+myRange(ylim)*offset;%Anup changed here
